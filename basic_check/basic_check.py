...
def run_basic_checks(final_array, headers):
    ...
    for i, row in enumerate(final_array):
        ...
        check_0401_2(row, i, results, get)
...
# ④①② 特殊作業手当複数
def check_0401_2(row, i, results, get):

    # 次の行が存在する場合のみ
    if i + 1 < len(final_array):

        if get(row, "特殊作業手当") != "" and get(final_array[i+1], "特殊作業手当") != "":
...
def check_0403(row, i, results, get):
    ...
    if get(row, "泊り") != "" and get(row, "特殊作業手当") != "":
